...
export function ContactFaqSection() {
  return (
    <section className="border-t border-pelagic-sand bg-pelagic-sand/40 py-20 lg:py-24">
        ...
        <div className="mt-10 space-y-3">
          {contactPage.faq.map((item, i) => (
            <Reveal key={item.question} delay={i * 40}>
              <details className="group rounded-2xl border border-pelagic-sand bg-white shadow-sm">
                <summary className="cursor-pointer list-none px-5 py-4 font-semibold text-pelagic-ink marker:content-none sm:px-6">
                  <span className="flex items-center justify-between gap-4">
                    {item.question}
                    <span
                      className="shrink-0 text-pelagic-gold transition group-open:rotate-45"
                      aria-hidden
                    >
                      +
                    </span>
